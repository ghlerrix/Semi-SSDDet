...
class SemiDetector(BaseDetector):
    """
    Args:
    ...
    def loss_by_gt_instances(self, batch_inputs: Tensor,
        ...
        Returns:
            dict: A dictionary of loss components
        """
        self.student.bbox_head.mode = 'sup'
        
        losses = self.student.loss(batch_inputs, batch_data_samples)
        sup_weight = self.semi_train_cfg.get('sup_weight', 1.)
        return rename_loss_dict('sup_', reweight_loss_dict(losses, sup_weight))
    
    def cal_ada_thr(self,
                    data_samples: SampleList,
                    device):
        scores = data_samples.gt_instances.scores.to(device)
        labels = data_samples.gt_instances.labels.to(device)

        t_thrs = torch.zeros_like(scores, device=device)
        ...
        for label in unique_labels:
            ...
            if len(label_scores) < 4:
                pos_thr = self.semi_train_cfg.cls_pseudo_thr
                
            else:
                # 在GPU上使用cuML进行GMM处理
                label_scores_cpu = label_scores.cpu().numpy()[:, np.newaxis]

                # 在GPU上拟合GMM
                gmm = BayesianGaussianMixture(n_components=2, random_state=0)
                gmm.fit(label_scores_cpu)

                gmm_assignment = gmm.predict(label_scores_cpu)
                gmm_scores = gmm.score_samples(label_scores_cpu)

                # 找到最佳阈值
                if (gmm_assignment == 1).any():
                    gmm_scores[gmm_assignment == 0] = -np.inf
                    indx = np.argmax(gmm_scores)
                    pos_indx = (gmm_assignment == 1) & (label_scores_cpu.squeeze() >= label_scores_cpu[indx])
                    pos_thr = float(label_scores_cpu[pos_indx].min())
                    pos_thr = max(self.semi_train_cfg.cls_pseudo_thr, pos_thr)
                else:
                    pos_thr = self.semi_train_cfg.cls_pseudo_thr

            # 在GPU上更新阈值
            t_thrs[label_mask] = pos_thr
        return t_thrs
    ...
    def loss_by_pseudo_instances(self,
        ...
        t_thrs_list = torch.tensor([])
        # 遍历处理每个数据样本
        for index, data_samples in enumerate(batch_data_samples):
            t_thrs = self.cal_ada_thr(data_samples, data_samples.gt_instances.labels.device)
            if t_thrs_list.numel() == 0:
                t_thrs_list = t_thrs
            else:
                t_thrs_list = torch.cat((t_thrs_list, t_thrs))
            
            for label, thr in zip(torch.unique(data_samples.gt_instances.labels), t_thrs):
                if data_samples.gt_instances.bboxes.shape[0] > 0:
                    t_gt_instances = data_samples.gt_instances[data_samples.gt_instances.labels == label]
                    batch_data_samples[index].gt_instances = t_gt_instances[
                        t_gt_instances.scores > thr]

        message_hub = MessageHub.get_current_instance()
        message_hub.update_scalar('train/cls_thr', t_thrs_list.mean())
        
        self.student.bbox_head.mode = 'unsup'
        
        losses = self.student.loss(batch_inputs, batch_data_samples)
        # print('asdasdasdasdasdas', self.student.bbox_head.mode)
